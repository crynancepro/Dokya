...
Génère la version enrichie, professionnelle et optimisée ATS au format JSON.`;
      ...
      // Standalone Universal Letter Prompt
...
Ta mission est de rédiger une lettre officielle complète, hautement soignée, élégante et conforme aux normes en vigueur, occupant toute la feuille A4 (entre 250 et 350 mots, au moins 250 mots au total).

Catégorie exacte de lettre : ${letterCategory.toUpperCase()} | Type de lettre : ${letterType} | Ton : ${tone}

CONSIGNES STRICTES DE RÉDACTION OFFICIELLE :
1. **Objet clair et professionnel** : Formuler un objet précis, formel et percutant conforme aux standards administratifs, juridiques et commerciaux (${targetJobOrSubject}).
2. **Salutation protocolaire adaptée** : Utiliser la formule d'appel protocolaire exacte selon le destinataire (ex: 'Madame, Monsieur le Directeur,', 'Monsieur le Président,', 'Madame, Monsieur le Responsable des Recrutements,', 'Madame, Monsieur,').
3. **Corps de texte développé en 4 paragraphes distincts (250-350 mots)** :
   - **Paragraphe 1 (Accroche / Contexte officiel)** : Exposer clairement le motif de la lettre, l'objet précis de la démarche ou l'intérêt pour ${recipient}.
   - **Paragraphe 2 (Développement / Arguments / Faits circonstanciés / Compétences)** : Développer de manière rigoureuse les arguments, faits précis, antécédents, justificatifs ou compétences techniques avec clarté et vocabulaire soutenu.
   - **Paragraphe 3 (Synergie / Modalités / Démarche constructive)** : Préciser les modalités d'application, le respect scrupuleux des procédures, l'apport mutuel ou la valeur ajoutée pour l'organisation.
   - **Paragraphe 4 (Conclusion & Demande formelle)** : Formuler expressément la demande d'instruction, de rendez-vous, d'entretien, de décharge ou de suite favorable avec mention de la disponibilité immédiate.
4. **Formule de politesse (Closing)** : Formule solennelle, élégante et respectueuse conforme aux usages officiels en Afrique de l'Ouest.
5. **Intégration impérative des consignes utilisateur** : ${userInstructions ? `Intègre fidèlement et scrupuleusement ces consignes dans le corps de texte : "${userInstructions}".` : `Assure une rédaction exemplaire et rigoureuse.`}
6. Fournis également 3 conseils pratiques ou juridiques adaptés pour optimiser la démarche.`;
...
- Coordonnées : ${cleanData.personalInfo.email} | Téléphone : ${cleanData.personalInfo.phone}
- Localisation : ${cleanData.personalInfo.city || 'Dakar'}, ${cleanData.personalInfo.country || 'Sénégal'}
- Destinataire : ${recipient}
- Objet visé : ${targetJobOrSubject}
- Ton de rédaction : ${tone}
- Contexte & Consignes particulières : ${userInstructions || 'Rédiger une lettre officielle complète et percutante selon les règles de l\'art'}
...
async function handlePaymentConfirmation(params: {
  ...
  if (existingTxData && (existingTxData.status === 'SUCCESS' || existingTxData.status === 'COMPLETED')) {
    console.log(`[handlePaymentConfirmation] Transaction ${txId} déjà traitée (SUCCESS). Annulation du double crédit.`);
    return {
      success: true,
      alreadyProcessed: true,
      transactionId: txId,
      amount: existingTxData.amount || amount || 0,
...
app.post('/api/moneyfusion/checkout', async (req, res) => {
  try {
    ...
    const returnUrl = `${appBaseUrl}/dashboard?payment=return&transactionId=${transactionId}&docId=${targetDocId || ''}&type=${resolvedType}&plan=${targetPlanId || ''}&amount=${targetAmount}`;
    ...
    if (apiKey || process.env.MONEYFUSION_API_URL) {
      try {
        ...
        console.warn('[Money Fusion Checkout] Réponse inattendue de l\'API Money Fusion:', data);
        return res.status(400).json({
          success: false,
          error: data.message || "Échec de l'initialisation du paiement chez Money Fusion."
        });
      } catch (apiErr: any) {
        console.error('[Money Fusion Checkout] Erreur lors de l\'appel API Money Fusion:', apiErr?.message);
    ...
    return res.status(500).json({
      success: false,
      error: "Service de paiement Money Fusion non configuré sur le serveur."
    });
  } catch (error: any) {
...
app.post('/api/webhooks/moneyfusion', async (req, res) => {
  ...
  try {
    ...
    const token = String(payload.token || payload.tokenPay || payload.orderId || payload.id || '').trim();
    ...
    // 1. VÉRIFICATION D'IDEMPOTENCE IMMÉDIATE (RÈGLE ABSOLUE) :
    if (transaction && (transaction.status === 'SUCCESS' || transaction.isProcessed === true)) {
      console.log(`[Webhook] Transaction ${searchId} déjà traitée (SUCCESS / isProcessed: true). Annulation du double crédit.`);
      return res.status(200).json({ message: "Transaction déjà traitée, aucun crédit ajouté" });
    }

    // 2. VÉRIFICATION STRICTE DIRECTE AUPRÈS DE L'API OFFICIELLE MONEY FUSION :
    // On ne se fie JAMAIS au boolean statut:true seul car Money Fusion renvoie statut:true pour 'pending'
    ...
    // Repli de secours STRICT sur le payload
    if (!isConfirmedPaid) {
    ...
    // Si le paiement n'est pas encore confirmé : AUCUN CRÉDIT
    if (!isConfirmedPaid) {
      return res.status(200).json({
        message: "Notification Money Fusion reçue. En attente de confirmation par l'opérateur, aucun crédit ajouté."
      });
    }
    ...
    // 4. EXÉCUTION ATOMIQUE AVEC VERROU FIRESTORE :
    const atomicResult = await executeAtomicPaymentCredit({
      searchId,
      effectiveUserId,
      effectiveAmount,
      userEmail: userEmail || transaction?.userEmail || '',
      userName: userName || transaction?.userName || 'Utilisateur',
      phoneNumber: userPhone || transaction?.phoneNumber || ''
    });
    ...
    return res.status(500).json({ error: error?.message || 'Erreur interne de traitement' });
  }
});
...
/**
 * 1.E. Route de vérification de retour client
 * GET / POST /api/moneyfusion/verify
 * RÈGLE D'OR : Ne crédite AUCUN solde. Cette page doit uniquement lire la transaction Firestore
 * pour afficher un message de succès à l'utilisateur.
 */
app.all('/api/moneyfusion/verify', async (req, res) => {
  try {
    ...
    const searchId = transactionId || token;
    console.log('[Server /api/moneyfusion/verify] Lecture seule Firestore pour:', searchId);
    ...
    const finalAmount = Number(txData?.amount || txData?.expectedAmount || amount || 0);
    ...
    return res.json({
      ...
      message: isPaid ? 'Paiement validé avec succès' : 'Paiement en attente de confirmation par votre opérateur Mobile Money'
    });
  } catch (err: any) {
